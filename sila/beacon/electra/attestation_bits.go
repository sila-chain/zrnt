...
func (li *AttestationBits) Deserialize(spec *common.Spec, dr *codec.DecodingReader) error {
	return dr.BitList((*[]byte)(li), uint64(spec.MAX_VALIDATORS_PER_COMMITTEE*spec.MAX_COMMITTEES_PER_SLOT))
}
...
func (li AttestationBits) HashTreeRoot(spec *common.Spec, hFn tree.HashFn) common.Root {
	return hFn.BitListHTR(li, uint64(spec.MAX_VALIDATORS_PER_COMMITTEE*spec.MAX_COMMITTEES_PER_SLOT))
}
...
func AttestationBitsType(spec *common.Spec) *BitListTypeDef {
	return BitListType(uint64(spec.MAX_VALIDATORS_PER_COMMITTEE * spec.MAX_COMMITTEES_PER_SLOT))
}
